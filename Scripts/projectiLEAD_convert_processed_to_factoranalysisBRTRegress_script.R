...
write.table(ace_cfa, file = paste0("Data/proj-InitialPapers_asmt-A_time-1234_type-agg_shape-long_clean_trimRTAccExOutlierRMBRTRegress_prog-Mplus_date-", Sys.Date(), '.csv'), sep = ',', row.names = F, col.names = F)
#And the column names for reference
write.table(names(ace_cfa), file = paste0("Data/colnames_proj-InitialPapers_asmt-A_time-1234_type-agg_shape-long_clean_trimRTAccExOutlierRMBRTRegress_prog-Mplus_date-", Sys.Date(), '.txt'), sep = ',', row.names = F, col.names = F)
...
write.table(ace_wide, file = paste0("Data/proj-InitialPapers_asmt-A_time-1234_type-agg_shape-wide_clean_trimRTAccExOutlierRMBRTRegress_prog-Mplus_date-", Sys.Date(), '.csv'), sep = ',', row.names = F, col.names = F)
#And the column names for reference
write.table(names(ace_wide), file = paste0("Data/colnames_proj-InitialPapers_asmt-A_time-1234_type-agg_shape-wide_clean_trimRTAccExOutlierRMBRTRegress_prog-Mplus_date-", Sys.Date(), '.txt'), sep = ',', row.names = F, col.names = F)
