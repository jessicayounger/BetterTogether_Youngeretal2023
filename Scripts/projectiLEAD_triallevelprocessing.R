...
#Last updated by Jessica Younger 2023-09-20
...
db=read.csv(file="Data/Database_ImputedAges_RemoveNames_2023-09-20.csv")
...
#Merge with additional demographic variables available in the Database
...
#Exclude the FILTER task due to differential challenge level across Cohorts and TASKSWITCH due to technical error at T1 
data=data%>%
  select(-contains("FILTER"), -contains("TASKSWITCH")) 
...
outliers=data_initialclean%>%
  ...
  select(BOXED.rt_mean.correct, SAATSUSTAINED.rt_sd.correct, SAATIMPULSIVE.sdt_dprime.overall, BRT.rt_mean.correct, STROOP.rcs.overall, FLANKER.rcs.overall, TNT.sdt_dprime.tap_trace, SPATIALSPAN.object_count_span.overall, BACKWARDSSPATIALSPAN.object_count_span.overall)%>%
...
#Stroop
mod <- lmer(STROOP.rcs.overall ~time_since_firstgameplay * age_years + gender+Cohort+ BRT.rt_mean.correct + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
...
#Flanker
mod <- lmer(FLANKER.rcs.overall ~time_since_firstgameplay * age_years+ gender +Cohort+ BRT.rt_mean.correct + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
is.na(compiled[influential_check, grep("FLANKER", names(compiled))])=TRUE

#Backward Spatial Span
mod <- lmer(BACKWARDSSPATIALSPAN.object_count_span.overall ~time_since_firstgameplay * age_years + gender  +Cohort+ BRT.rt_mean.correct + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
...
#Spatial Span
mod <- lmer(SPATIALSPAN.object_count_span.overall ~time_since_firstgameplay * age_years+ gender +Cohort+ BRT.rt_mean.correct + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
...
#Boxed
mod <- lmer(BOXED.rt_mean.correct ~time_since_firstgameplay * age_years +gender +Cohort+ BRT.rt_mean.correct + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
is.na(compiled[influential_check, grep("BOXED", names(compiled))])=TRUE

#TNT
mod <- lmer(TNT.sdt_dprime.tap_trace ~time_since_firstgameplay * age_years+ gender +Cohort+ BRT.rt_mean.correct.dominant + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
...
#SAAT impulsive
mod <- lmer(SAATIMPULSIVE.sdt_dprime.overall ~time_since_firstgameplay * age_years +  gender  +Cohort+ BRT.rt_mean.correct.dominant + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
...
#SAAT sustained
mod <- lmer(SAATSUSTAINED.rt_sd.correct ~time_since_firstgameplay * age_years  + gender +Cohort+ BRT.rt_mean.correct.dominant + (1 + time_since_firstgameplay|pid) + (1|School), data=compiled)
cooksd <- cooks.distance(mod)
influential_check <- as.numeric(names(cooksd)[(cooksd > 1)])  # influential row numbers
is.na(compiled[influential_check, grep("SUSTAINED", names(compiled))])=TRUE
#Note: error occurs, as there are no influential observations

#Due to privacy concerns, the code for cleaning outliers to Cooks d was changed from age_months to age_years which results in slightly different influential datapoints.
#To better replicate the data, the following code aligns the data points across the two analyses, adding back data that was not removed when the age_months variables was used and removing data that was removed when age_months was used

compiled_replication=compiled
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ff701" & compiled_replication$Time.Point=="3", "STROOP.rcs.overall"] = data_clean[data_clean$pid=="ADMIN-UCSF-ff701" & data_clean$Time.Point=="3", "STROOP.rcs.overall"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-dd086" & compiled_replication$Time.Point=="2", "FLANKER.rcs.overall"] = data_clean[data_clean$pid=="ADMIN-UCSF-dd086" & data_clean$Time.Point=="2", "FLANKER.rcs.overall"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-dd422" & compiled_replication$Time.Point=="4", "FLANKER.rcs.overall"] = data_clean[data_clean$pid=="ADMIN-UCSF-dd422" & data_clean$Time.Point=="4", "FLANKER.rcs.overall"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ff088" & compiled_replication$Time.Point=="2", "FLANKER.rcs.overall"] = data_clean[data_clean$pid=="ADMIN-UCSF-ff088" & data_clean$Time.Point=="2", "FLANKER.rcs.overall"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-aa096" & compiled_replication$Time.Point=="1", "BOXED.rt_mean.correct"] = data_clean[data_clean$pid=="ADMIN-UCSF-aa096" & data_clean$Time.Point=="1", "BOXED.rt_mean.correct"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-bb059" & compiled_replication$Time.Point=="2", "BOXED.rt_mean.correct"] = data_clean[data_clean$pid=="ADMIN-UCSF-bb059" & data_clean$Time.Point=="2", "BOXED.rt_mean.correct"]
compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ii010" & compiled_replication$Time.Point=="2", "BOXED.rt_mean.correct"] = data_clean[data_clean$pid=="ADMIN-UCSF-ii010" & data_clean$Time.Point=="2", "BOXED.rt_mean.correct"]
is.na(compiled_replication[compiled_replication$pid=="ADMIN-UCSF-dd411" & compiled_replication$Time.Point=="4", "BOXED.rt_mean.correct"])=TRUE
is.na(compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ff657" & compiled_replication$Time.Point=="3", "BOXED.rt_mean.correct"])=TRUE
is.na(compiled_replication[compiled_replication$pid=="ADMIN-UCSF-gg408" & compiled_replication$Time.Point=="4", "STROOP.rcs.overall"])=TRUE
is.na(compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ff057" & compiled_replication$Time.Point=="1", "STROOP.rcs.overall"])=TRUE
is.na(compiled_replication[compiled_replication$pid=="ADMIN-UCSF-ff627" & compiled_replication$Time.Point=="3", "FLANKER.rcs.overall"])=TRUE

#######Final Variable Selection########
final_data=compiled_replication%>%
  select(pid, Cohort, Time.Point, grade, gender, contains ("span"),SAATSUSTAINED.rt_sd.correct, SAATIMPULSIVE.sdt_dprime.overall, TNT.sdt_dprime.tap_trace, contains("rcs.overall"),BOXED.rt_mean.correct, BRT.rt_mean.correct, BRT.rt_mean.correct.dominant, BRT.rt_mean.correct.nondominant,  School, contains("time"), age_years, Birthdate_provided)
...
nremoved=ACE_datasetscleaning(variables=c("STROOP.rcs.overall", "FLANKER.rcs.overall", "SPATIALSPAN.object_count_span.overall", "BACKWARDSSPATIALSPAN.object_count_span.overall",
                                          "SAATSUSTAINED.rt_sd.correct", "SAATIMPULSIVE.sdt_dprime.overall", "TNT.sdt_dprime.tap_trace", "BOXED.rt_mean.correct"))
nremoved
