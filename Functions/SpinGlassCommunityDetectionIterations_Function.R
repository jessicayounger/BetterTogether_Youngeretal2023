##Function to plot multiple iterations in a standard format. 
...
spinComRec <- function(graphQgraph, # qgraph object
  ...
  for (yIt in 1:numberIterations){
    ...
    #set.seed(3+yIt) #Seed is set in the main script
    clusterStabilitySpin <- list()
    for (i in 1:numberEstimations){
      #set.seed(3+i) #Seed is set in the main script
    ...
    lalaName <- paste(outpath, "AllSpinglassCommunities", yIt, ".pdf", sep = "")
    ...
    for (i in 1:ncol(uniqueMembershipVectors)){
      ...
      #print the percentage of times this community was selected 
      ...
      # text(1,1.2, labels=mean(clusterStabilityModularity[equalMatrixList[[i]]]))
    }
    dev.off()
    
    # same as above but next to each other on one page (i.e., in groups of 5)
    lalaName <- paste(outpath, "AllSpinglassCommunitiesOnepage", yIt, ".png", sep = "")
    #Print to png because margins weren't working on PDF
    png(file=lalaName, units="in", width=10, height=10, res=300)
    #pdf(lalaName, width = 6, height = 6)
    par(mfrow = c(5,4),oma=c(12,0,0,0), xpd=NA)
    
    for (i in 1:ncol(uniqueMembershipVectors)){
      communitySGPlot1$membership <- uniqueMembershipVectors[,i]
      plot(communitySGPlot1, graphIgraph, layout = graphQgraph$layout,scale=T, labels=labels, vertex.size=45, vertex.label.cex=1, mark.border="black", rescale=F )
      #print the percentage of times this community was selected 
      text(1,1.5, labels = paste0((sapply(equalMatrixList, length)[i])/length(clusterStabilitySpin)*100, "%"))
      # text(1,1,labels = length(unique(uniqueMembershipVectors[,i])))
      # text(1,1.2, labels=mean(clusterStabilityModularity[equalMatrixList[[i]]]))
    }
    dev.off()
